...
class UNet_VAE(nn.Module):
    def __init__(self, num_classes, segment, in_channels=3, depth=3, 
        ...
        self.reset_params()
        self.result_tuple = collections.namedtuple("unet_vae_recon_result", ["y_pred"])
    ...
    def forward(self, x):
        ...
        # y_pred = rearrange(x, "(b t) c h w -> b t c h w", t=seq_len)
        x = self.conv_final(x)

        # x_segment = self.conv_final(x)
        x_recon = F.relu(x)
