...
class UNet_VAE_Block(nn.Module):
    def __init__(self, num_classes, segment, alpha, in_channels=5, depth=3, 
                 start_filts=64, up_mode='upsample', 
                 merge_mode='concat', enc_out_dim=256, latent_dim=100):
        """
        Arguments:
        ...
        self.conv_start = conv1x1(192, self.in_channels)
        ...
        self.fc1 = nn.Linear(enc_out_dim * 8 * 8, latent_dim)
        self.fc2 = nn.Linear(enc_out_dim * 8 * 8, latent_dim)
        self.fc3 = nn.Linear(latent_dim, enc_out_dim * 8 * 8)
        self.act = nn.ReLU()
    ...
    def forward(self, x):
        ...
        for i, module in enumerate(self.down_convs):
            ...
            if i == 0 and x.size(1) != 5:
                x = self.conv_start(x)

            # print(f'x shape: {x.shape} at {i} layer')
            x, s = module(x)
            s_dict[i] = s   
...
class UNet_VAE_Stacked(nn.Module):
    def __init__(self, num_classes, segment, alpha, device, model_weight, in_channels=5, unet_num_block=5):
        """
        Arguments:
        """
        ...
        # print("model weight file path: ",model_weight)

        for i in range(self.unet_num_block):
            block = UNet_VAE_Block(num_classes, segment, alpha, in_channels = self.in_channels)
            model_dict = block.state_dict()
            print('Block state dict')
            for var_name in model_dict:
                print(var_name, "\t")
            if model_weight is not None:
                # get dictionary of pretrained UNet-VAE weight
                pretrained_dict = torch.load(model_weight)
                print('Pretrained state dict')
                # for var_name in pretrained_dict:
                #     print(var_name, "\t")
                # 1. filter out unnecessary keys
                # pretrained_dict = {k.replace("encoder.", "", 1): v for k, v in pretrained_dict.items() if k.replace("encoder.", "", 1) in model_dict}
                # pretrained_dict = {k.replace("encoder.", "", 1): v for k, v in pretrained_dict.items()}
                for var_name in pretrained_dict:
                    print(var_name, "\t")
                ...
                # block.load_state_dict(model_dict, strict=False)
                block.load_state_dict(pretrained_dict, strict=False)
                block.to(device)
            self.unet_blocks.append(block)

        self.unet_blocks = nn.ModuleList(self.unet_blocks)
        # self.conv_final = conv1x1(64, self.num_classes)
    ...
    def forward(self, x):
        ...
        # print(f'x stacked final shape: {x.shape}')
        #x_recon = F.relu(x)
