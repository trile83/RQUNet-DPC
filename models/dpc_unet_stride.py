...
def stride_over_time(x):
    ...
    # c1 = nn.Conv3d(in_channels=128, out_channels=64, kernel_size=3, padding=1)
    # c2 = nn.Conv3d(in_channels=64, out_channels=32, kernel_size=3, padding=1)
    # c3 = nn.Conv3d(in_channels=32, out_channels=16, kernel_size=3, padding=1)
    # c4 = nn.Conv3d(in_channels=16, out_channels=8, kernel_size=3, padding=1)
    # c5 = nn.Conv3d(in_channels=8, out_channels=1, kernel_size=3, padding=1)

    # out1 = c1(x)            # batch-size x 64 x timesteps x height x width
    # out2 = c2(out1)         # batch-size x 32 x timesteps x height x width
    # out3 = c3(out2)         # batch-size x 16 x timesteps x height x width
    # out4 = c4(out3)         # batch-size x 8 x timesteps x height x width
    # out5 = c5(out4)         # batch-size x 1 x timesteps x height x width
    # out6 = out5.squeeze(1)  # batch-size x timesteps x height x width
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            ...
            save_checkpoint({'epoch': epoch+1,
                            ...
                            is_best, filename=os.path.join(model_dir, 'dpc_pad_stridetime_13band_epoch%s.pth' % str(epoch+1)), keep_all=False)
            ...
            save_checkpoint({'epoch': epoch+1,
                            ...
                            is_best, filename=os.path.join(model_dir, 'unetsegment_pad_stridetime_13band_epoch_%s.pth' % str(epoch+1)), keep_all=False)

    plt.plot(train_loss_lst, color ="blue")
    plt.plot(val_loss_lst, color = "red")
    plt.savefig("/home/geoint/tri/dpc_test_out/train_dpc_cnn_loss.png")
    plt.close()
...
def train_segment(data_loader, segment_model, optimizer):
    ...
    for idx, input in enumerate(data_loader):
        ...
        #### using convolution to stride through temporal dimension
        x = input['x']
