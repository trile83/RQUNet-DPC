...
import numpy as np
import cv2
sys.path.append('/home/geoint/tri/DPC/DPC-master')
...
class UCF101_3d(data.Dataset):
    def __init__(self,
        ...
        # splits
        if mode == 'train':
            split = '/home/geoint/tri/DPC/DPC-master/process_data/data/ucf101/train_split%02d.csv' % self.which_split
            video_info = pd.read_csv(split, header=None)
        elif (mode == 'val') or (mode == 'test'): # use val for test
            split = '/home/geoint/tri/DPC/DPC-master/process_data/data/ucf101/test_split%02d.csv' % self.which_split 
            video_info = pd.read_csv(split, header=None)
        else: raise ValueError('wrong mode')
        ...
        self.action_dict_encode = {}
        self.action_dict_decode = {}
        #action_file = os.path.join('/home/geoint/tri/DPC/DPC-master/process_data/data/ucf101', 'classInd.txt')
        action_file = '/home/geoint/tri/DPC/DPC-master/process_data/data/ucf101/classInd.txt'
        action_df = pd.read_csv(action_file, sep=' ', header=None)
        for _, row in action_df.iterrows():
            act_id, act_name = row
            self.action_dict_decode[act_id] = act_name
