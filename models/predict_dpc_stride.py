...
def main():
    ...
    ### 13 bands
    dpc_checkpoint = f'{str(model_dir)}dpc_pad_stridetime_13band_epoch70.pth'
    unetsegment_checkpoint = f'{str(model_dir)}unetsegment_pad_stridetime_13band_epoch_70.pth'
    ...
    with open(f'{data_dir}{ts_name}_stat_results.csv','w') as f1:
        ...
        for idx, batch in enumerate(train_segment_dl):
            x1 = stride_over_channels(batch['x'])
            x = x1.to(cuda, dtype=torch.float32)
            y = batch['mask'].numpy()
            z = batch['ts'].numpy()
            ...
            plt.imshow(image)
            plt.savefig(f"{str(data_dir)}{ts_name}_{str(idx)}_stride.png")
