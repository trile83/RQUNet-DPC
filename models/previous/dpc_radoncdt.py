...
def main():
    ...
    (I,L,C,H,W)  = train_ts_set.shape

    ## add Radon-CDT forward function
    # Create the signal

    ###### Perform transform
    x0_range = [0, 1]
    x_range = [0, 1]
    template=np.ones([input_size, input_size])
    print(template.shape)
    radoncdt = RadonCDT()

    radon_ts_set = np.zeros((I,L,C,91,180))

    for i in range(train_ts_set.shape[0]):
        for h in range(train_ts_set.shape[1]):
            for j in range(train_ts_set.shape[2]):
                I = train_ts_set[i,h,j,:,:]
                # print('I shape: ', I.shape)
                Ihat = radoncdt.forward(x0_range, template, x_range, I)
                # print('Ihat shape: ', Ihat.shape)
                radon_ts_set[i,h,j,:,:] = Ihat
    
    # radon_ts_set = np.asarray(radon_ts_set)

    ######### Done with the radon-cdt forward

    print(f"train ts set shape: {radon_ts_set.shape}")
    print(f"train mask set shape: {train_mask_set.shape}")

    im_set = get_seq(radon_ts_set, seq_length) #(I,L1,SL,C,H,W)
    ...
    ## Perform reverse Radon-cdt

    for i in range(all_feature_arr.shape[0]):
        for h in range(all_feature_arr.shape[1]):
            for j in range(all_feature_arr.shape[2]):
                Ihat = all_feature_arr[i,h,j]
                all_feature_arr[i,h,j] = radoncdt.inverse(Ihat, template, x_range)

    print(f'inversed radon-cdt context vector shape: {all_feature_arr.shape}')
