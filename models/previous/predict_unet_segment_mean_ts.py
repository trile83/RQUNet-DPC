...
def chipper(ts_stack, mask, input_size):
    ...
    Return:
        output stack with chipped size
    '''
...
if __name__ == '__main__':
    ...
    for i in range(iteration):
        ...
        temp_ts_mean_set.append(ts_mean)
        temp_mask_set.append(mask)

    train_ts_set = np.stack(temp_ts_set, axis=0)
    train_ts_mean_set = np.stack(temp_ts_mean_set, axis=0)
    train_mask_set = np.stack(temp_mask_set, axis=0)

    print(f"train ts set shape: {train_ts_set.shape}")
    print(f"train ts mean set shape: {train_ts_mean_set.shape}")
    print(f"train mask set shape: {train_mask_set.shape}")

    test_set = satDataset(train_ts_mean_set , train_mask_set, train_ts_set)

    # 3. Create data loaders
    batch_size = 1
    loader_args = dict(batch_size=batch_size, num_workers=4, pin_memory=True, drop_last=True, shuffle=True)
    ...
    model_weights = str(dir_checkpoint)+'reconstruct_13band_unetvae_hls_93_0.0009755486249923706.pth'
    ...
    for idx, batch in enumerate(train_dl):
        ...
        for j in range(batch_size):
            for ts_index in range(z.shape[1]):
                plt.figure(figsize=(20,20))
                plt.subplot(1,3,1)
                plt.title("Image")
                image = np.transpose(batch['ts'].numpy()[j,ts_index,:3,:,:], (1,2,0))  
                plt.imshow(rescale_truncate(image))
                plt.subplot(1,3,2)
                plt.title("Segmentation Label")
                image = np.transpose(batch['mask'].numpy()[j,:,:], (0,1))
                plt.imshow(image)
            
                plt.subplot(1,3,3)
                plt.title("Segmentation")
                image = np.transpose(index_array[j,:,:].cpu().numpy(), (0,1))
                plt.imshow(image)
                plt.savefig(f"{str(data_dir)}{str(idx)}_{str(j)}_ts_{ts_index}.png")
