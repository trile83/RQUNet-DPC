...
def main():
    ...
    dir_checkpoint = '/home/geoint/tri/dpc_test_out/checkpoints/'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train_loss = train_segment(train_dl,unet_segment, optimizer)
        loss_item = 0.0
        ...
        for idx, input in enumerate(test_dl):
            ...
            input_seq = input["x"].cuda()
            input_mask = input["binary_activity_mask"].cuda()
            # print(f"input mask shape: {input_mask.shape}")
            (B,T,C,H,W) = input_seq.shape
            # input_mask = input_mask.view(B,T,H,W)
            # print(f'input mask shape: {input_mask.shape}')

            # mask_one_hot = F.one_hot(input_mask)

            B = input_seq.size(0)

            mask_pred = unet_segment(input_seq)

            # print(f"mask pred shape: {mask_pred.shape}")

            # input_mask = rearrange(input_mask, "b t h w -> (b t) h w")
            # mask_pred = rearrange(mask_pred, "b t c h w -> (b t) c h w")
        ...
        epoch_loss = loss_item/len(test_dl)
        ...
        if min_loss > epoch_loss:
            ...
            # Saving State Dict
            torch.save(unet_segment.state_dict(), dir_checkpoint + f'segment_BH_R001_unet3d_{criterion_type}_loss_{epoch}_{min_loss}.pth')
    ...
    plt.savefig('/home/geoint/tri/dpc/models/output/unet3d_semantic_train_loss_new.png')
    plt.close()
