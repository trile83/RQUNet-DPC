...
parser.add_argument('--seq_len', default=2, type=int, help='number of frames in each video block')
parser.add_argument('--num_seq', default=5, type=int, help='number of video blocks')
...
def get_seq(sequence, seq_length):
    ...
    for j in range(I):
        for i in range(seq_length, L+1,sequence):
...
def get_chunks(windows, num_seq):
    ...
    for j in range(I):
        for i in range(num_seq, L1+1,num_seq):
...
def main():
    ...
    out_train_file = f'{out_data_dir}train_dpc_data_{list_ts[0]}_{args.channels}band.npy'
    out_val_file = f'{out_data_dir}val_dpc_data_{list_ts[0]}_{args.channels}band.npy'
    ...
    if network == 'unet-vae' or network == 'rqunet-vae' or network == 'unet':
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if is_best:
                ...
                plt.imshow(image)
                plt.savefig(f"/projects/kwessel4/dpc/output/image/training-{str(epoch)}-{str(idx)}-dpc-unet-{today}-plot.png")
                plt.close()
                ...
                save_checkpoint({'epoch': epoch+1,
                                ...
                                is_best,
                                filename=os.path.join(model_dir, f'dpc-{network}-{today}-{criterion_type}_{args.segment_model}_std_{args.standardization}_{args.hidden_dim}_{np.round(min_loss,3)}_0.2binary_{args.channels}band_18_epoch%s.pth' % str(epoch+1)),
                                keep_all=False)
